...
const ProductsListAdmin = () => {
  ...
  const validarCampos = useCallback(
    ({ title, price, description, category, image, stock }) => {
      ...
      if (image?.trim() && !/^https?:\/\/.*\.(jpg|jpeg|png|webp|gif|svg|png)$/i.test(image)) {
  ...
  const handleGuardarEdicion = useCallback(
    async (id) => {
      ...
      try {
        ...
        toast.dismiss(loadingToast);
        toast.success(
          `Producto "${productoData.title}" actualizado exitosamente`
        );
        setEditando(null);
      } catch (error) {
  ...
  const FormularioProducto = useMemo(
    () =>
      ({ producto, onChange, onSubmit, submitText, onCancel }) =>
        (
          <div className="form-producto">
            ...
            <input
              type="url"
              placeholder="URL de imagen (sube en imgbb.com para obtener enlace)"
              value={producto.image}
              onChange={(e) => onChange("image", e.target.value)}
              title="Sube tu imagen en imgbb.com y pega el enlace directo aquí"
            />
